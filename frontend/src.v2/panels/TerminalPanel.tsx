...
export const TerminalPanel = () => {
  ...
  useEffect(() => {
    ...
    const unsub = ws?.subscribe((msg) => {
      const e = msg as { type?: string; session_id?: string; data?: string; output?: string; command?: string; exit_code?: number };
    ...
    if (isDesktop()) {
      const d = desktop();
      desktopDataCleanup = d?.pty.onData(({ sessionId, data }) => appendOutput(sessionId, data)) as (() => void) | undefined;
  ...
  const refreshSessions = async () => {
    ...
    try {
      if (isDesktop()) {
        ...
        useAppStore.getState().setTerminalSessions(sessions.map((session) => ({
  ...
  return (
    ...
    >
      <div style={toolbarStyle}>
        <button type="button" onClick={() => void createSession()} style={titleIconButtonStyle} title="New terminal" aria-label="New terminal">
          <Plus size={14} />
        </button>
        {liveUrl && (
          <button
      ...
      <div
        ref={containerRef}
        style={terminalViewportStyle(themeMode === "light" || document.documentElement.getAttribute("data-theme") === "light")}
      />
    </div>
  );
};
...
const terminalViewportStyle = (isLight: boolean): React.CSSProperties => ({
  flex: 1,
  width: "100%",
  minHeight: 0,
  padding: 2,
  background: terminalTheme(isLight).background,
