...
from backend.config import _load_settings_json, _write_settings_json, load_config
...
def get_feature_flag_settings() -> dict[str, Any]:
    ...
    effective = feature_flags_payload(settings_data)
    flags: list[dict[str, Any]] = []
    for name in sorted(DEFAULT_FEATURE_FLAGS):
        env_name = feature_env_name(name)
        effective_item = effective.get(name, {})
        source = "env" if env_name in os.environ else "settings" if name in overrides else "default"
        flags.append({
            "name": name,
...
async def update_feature_flag_settings(
    ...
    *,
    ...
    await config_change_hook(source="feature_flags", file_path=str(settings_file))
    return {
