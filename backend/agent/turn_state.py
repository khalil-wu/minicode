...
class AgentTurnState:
    ...
    def commit_text(self, content: str) -> None:
        """
        Commit final answer content as fallback when streaming was interrupted.
        Only sets text if no text blocks exist yet (streaming deltas take priority).
        """
        if not content:
            return
        # If we already have text blocks from streaming deltas, don't overwrite
        if any(block.get('type') == 'text' for block in self._blocks):
            return
        # Fallback: use committed content
        self._blocks.append({'type': 'text', 'content': content})
